...
############# file path functions
    ...
    def saveFileDialog(self):
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(self,"Save parameter file (.yaml)","","All Files (*);;YAML Files (*.yaml)")
        return fileName
...
############# SSM on click buttons
    ...
    def SSM_register(self, params = True):
        if os.path.exists(self.SSM_source_fp) == False:
            ...
            with open(os.path.join(self.SSM_wd,'SSM_'+ts + project_name + '_config.yaml'), 'w') as outfile:
...
#            intermediate_output = intermed
            ...
            if params == False:
                ...
                print("Source -> Target 1 using registration model : " + self.SSM_reg_model1)
                print("Target1 -> Target 2 using registration model : " + self.SSM_reg_model2)
                ...
                register_SSM(self.SSM_source_fp, source_res,
                             ...
                             self.SSM_reg_model1, self.SSM_reg_model2,
                             project_name,
    ...
    def SSS_register(self, params = True):
        if os.path.exists(self.SSS_source_fp) == False:
            ...
            if params == False:
                ...
                print("Source -> Target using registration model : " + self.SSS_reg_model1)

                register_SSS(self.SSS_source_fp, source_res,
                self.SSS_target_fp, target_res,
                self.SSS_src_mask_fp, self.SSS_tgt_mask_fp,
                self.SSS_wd,
...
############# MSM on click buttons
    ...
    def MSM_register(self, params = True):
        if os.path.exists(self.MSM_source_fp) == False:
            ...
            with open(os.path.join(self.MSM_wd,'MSM_'+ts + project_name + '_config.yaml'), 'w') as outfile:
            ...
            if params == False:
                ...
                print("Source -> Target 1 using registration model : " + self.MSM_reg_model1)
                print("Target1 -> Target 2 using registration model : " + self.MSM_reg_model2)

                register_MSM(self.MSM_source_fp,source_res ,
                self.MSM_target1_fp, target1_res,
                self.MSM_target2_fp, target2_res,
                self.MSM_src_mask_fp, self.MSM_tgt1_mask_fp, self.MSM_tgt2_mask_fp,
                self.MSM_wd,
                MSM_source_img_type, MSM_target_img_type1, MSM_target_img_type2,
                self.MSM_reg_model1, self.MSM_reg_model2,
                project_name,
                intermediate_output = intermed,
                pass_in_project_name=True, pass_in = ts + project_name)
...
############# HDR on click buttons
    ...
    def HDR_register(self):
        if os.path.exists(self.HDR_source_fp) == False:
            ...
            bruker_output_xmls(self.HDR_source_fp, self.HDR_target_fp,
            self.HDR_wd, self.HDR_ijrois_fp,
            project_name,
...
##load params functions
    ...
    def MSM_oc_load_param(self):
        ...
        if param_map['param_mode'] != 'MSM':
            print('The config file specified is the incorrected type:' + param_map['param_mode'])
    ...
    def SSS_oc_load_param(self):
        ...
        if param_map['param_mode'] != 'SSS':
            ...
            index = self.ui.SSS_target_img_type.findText(param_map['target_img_type'], QtCore.Qt.MatchFixedString)
            if index >= 0:
    ...
    def MSS_oc_load_param(self):
        ...
        if param_map['param_mode'] != 'MSS':
            ...
            index = self.ui.MSS_target_img_type.findText(param_map['target_img_type'], QtCore.Qt.MatchFixedString)
            if index >= 0:
