...
class IMS_pixel_maps(object):
    ...
    def IMS_reg_mask(self, stamping = True):
        ...
        if stamping == True:
            ...
            stamp_mat = sitk.ConstantPad(stamp_mat, (self.img_padding, self.img_padding), (self.img_padding, self.img_padding))
            IMS_mask_upsampled = sitk.GetImageFromArray(sitk.GetArrayFromImage(stamp_mat) * sitk.GetArrayFromImage(IMS_mask_upsampled))
            del stamp_mat
            IMS_mask_upsampled = sitk.RescaleIntensity(IMS_mask_upsampled, 0, 255)
    ...
    def IMS_idxed_mask(self):
        IMS_mask_idx = np.zeros(( max(self.spots['y']) + 1, max(self.spots['x']) + 1))
        IMS_mask_idx[np.array(self.spots['y']), np.array(self.spots['x'])] =  np.arange(1,len(np.array(self.spots['x']))+1,1)
...
##testing:
#sl_pmap = IMS_pixel_maps('D:/20171127_malaria_data/IMS_data/Imzmls/20171128_muLiv_malaria_HE_targ_lesions_FI.imzML', 20, 1, 20)
...
#sl_pmap = IMS_pixel_maps(filepath , 100, 2, 20)
#sl_pmap.IMS_reg_mask(stamping=False)
