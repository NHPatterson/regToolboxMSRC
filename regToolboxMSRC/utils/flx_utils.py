...
class FI_ijrois(object):
    ...
    def draw_rect_mask(self, return_np = False):
        ...
        for i in range(len(self.roi_corners)):
            if i == 0:
                filled = cv2.rectangle(self.zero_image, (self.roi_corners[i][0][1],self.roi_corners[i][0][0]), (self.roi_corners[i][1][1],self.roi_corners[i][1][0]),(255), thickness= -1)
            else:
                filled = cv2.rectangle(filled, (self.roi_corners[i][0][1],self.roi_corners[i][0][0]), (self.roi_corners[i][1][1],self.roi_corners[i][1][0]),(255), thickness= -1)
    ...
    def get_rect_rois_as_images(self, image_fp):
        ...
        for i in range(len(self.allcoords)):
            roi_slices.append(bg_image[self.allcoords[i][0][1]:self.allcoords[i][1][1], self.allcoords[i][0][0]:self.allcoords[i][3][0]])
        self.roi_slices = []
        self.roi_slices.append(roi_slices)

    def get_index_and_overlap(self, ims_index_map_fp, ims_res, img_res):
        if self.polygons:
            ims_idx_np = sitk.GetArrayFromImage(sitk.ReadImage(ims_index_map_fp))
            scale_factor  = ims_res / img_res
            zero_img = np.zeros(ims_idx_np.shape[::-1])

            for i in range(len(self.polygons)):
                fill = cv2.fillConvexPoly(zero_img, self.polygons[i].astype(np.int32), i+1)
            ...
            for i in range(len(self.polygons)):
                whereresult = ims_idx_np[[np.where(fill == i+1)[0],np.where(fill == i+1)[1]]]

                uniques, counts = np.unique(whereresult, return_counts=True)

                df_intermed = pd.DataFrame({'roi_index': i+1, 'ims_index' : uniques, 'overlap' : counts / scale_factor**2})
...
def mask_contours_to_boxes(binary_mask):
    ...
    im2, contours, hierarchy = cv2.findContours(threshsrc,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
...
def split_boxes(roi_coords, no_splits = 4, base_name = "base", ims_res = "20", ims_method = "par", roi_name = "roi" ):
    ...
    for i in range(len(splits)):
        ...
        outputXMLboxes(splits[i], imsres= ims_res, imsmethod= ims_method, roiname= roi_name + "_split"+str(i)+"_", filename= base_name +"_" + str(i) + ".xml")

def outputXMLboxes(boundingRect_df, imsres="100",imsmethod="mymethod.par", roiname= "myroi_", filename="myxml.xml"):
    ...
    metadata_df = pd.DataFrame(np.arange(1, len(boundingRect_df)+1, 1),columns=['idx'])
    ...
    for j in range(len(boundingRect_df)):
        ...
        the_doc = ROOT('', Type="0",Name=metadata_df['name'] [j],Enabled="0",ShowSpectra="0",SpectrumColor=metadata_df['SpectrumColor'][j])
        ...
        areaxmls.append(lxml.etree.tostring(the_doc, pretty_print=True,encoding='unicode'))
...
def bruker_output_xmls(source_fp, target_fp, wd, ijroi_fp, project_name, ims_resolution = 10 , ims_method = "par", roi_name= "roi", splits = "0"):
    ...
    #parse csv file into flexImaging xml for RECTANGLES!!!! only!!
    outputXMLboxes(roi_coords, imsres= ims_resolution, imsmethod= ims_method, roiname= roi_name + "_", filename= ts + project_name + roi_name + ".xml")

    if no_splits > 1:
        split_boxes(roi_coords, no_splits = no_splits, base_name = ts + project_name + roi_name, ims_res = ims_resolution, ims_method = ims_method, roi_name = roi_name)
...
##testing:
#fp_moving = 'D:/testing_data/FI_ijrois_testing/171127 mouse liver malaria_BF_set1sec2.jpg'
#fp_fixed = 'D:/testing_data/FI_ijrois_testing/171127 mouse liver malaria_BF_set1sec2_0001.jpg'
#wd = 'D:/testing_data/FI_ijrois_testing'
#ijroi_fp = 'D:/testing_data/FI_ijrois_testing/lesion_rois.zip'
