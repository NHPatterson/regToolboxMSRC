...
def register_MSM(
    ...
    target1_res : float
        Image resolution of first target image image, specified in
        microns / pixel
    target2_fp : str
    ...
    Returns
    -------
        The function writes the transformation files and images
    ...
    if intermediate_output is True:
        tformed_im = transform_mc_image_sitk(
            target1_fp, tgt1_tgt2_tform_init, target1_res, override_tform=False
        )

        sitk.WriteImage(
            tformed_im,
            os.path.join(os.getcwd(), opdir, project_name + "_tgt1_tgt2_init.tif"),
            True,
        )
    ...
    target1 = reg_image_preprocess(
        init_img, target2_res, img_type="in_memory", mask_fp=None, bounding_box=False
    )
