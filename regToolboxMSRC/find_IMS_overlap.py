...
import time
import datetime
from regToolboxMSRC.utils.reg_utils import transform_image, RegImage_load, parameterFile_load
import SimpleITK as sitk
import pandas as pd
import numpy as np


def IMS_ablation_overlap(source_ims_fp,
                         target_ims_fp,
                         source_ims_res,
                         target_ims_res,
                         source_key_fp,
    ...
    if source_ims.image.GetPixelIDTypeAsString() != '32-bit unsigned integer':
        print(
            'Error: source image should have PixelType : 32-bit unsigned integer'
        )
        return
    ...
    if target_ims.image.GetPixelIDTypeAsString() != '32-bit unsigned integer':
        print(
            'Error: target image should have PixelType : 32-bit unsigned integer'
        )
        return
    ...
    df = pd.DataFrame({
        'source_idx': unique_source,
        'target_idx': unique_target,
        'percentage': counts
    })
...
if __name__ == '__main__':
    ...
    IMS_ablation_overlap(dataMap['source_ims_fp'], dataMap['target_ims_fp'],
                         dataMap['source_ims_res'], dataMap['target_ims_res'],
                         dataMap['source_key_fp'], dataMap['target_key_fp'],
                         dataMap['init_tform_fp'], dataMap['nl_tform_fp'],
                         dataMap['ims_res'], dataMap['img_res'],
                         dataMap['project_name'], dataMap['wd'])
