...
def register_SSM(
    ...
    """This function performs registration between 3 images from the same
    tissue section using linear transformation models.
    ...
    target1_res : float
        Image resolution of first target image image,
        specified in microns / pixel
    target2_fp : str
        String file path to second target image
    target2_res : float
        Image resolution of second target image image,
        specified in microns / pixel
    ...
    pass_in_project_name : boolean
        This parameter is used to pass in the project name
        from the reg_tlbx_gui
    pass_in : str
        Time stamped name fragment inherited from the GUI

    Returns
    -------
        The function writes the transformation files and
        images in the specified working directory.
    """
    ...
    if intermediate_output is True:
        ...
        sitk.WriteImage(
            tformed_im,
            os.path.join(os.getcwd(), opdir, project_name + "_src_tgt1.tif"),
            True,
        )
    ...
    tgt1_tgt2_tform = register_elx_n(
        target1,
        target2,
        reg_param2,
        output_dir=pass_in + "_tforms_tgt1_tgt2",
        output_fn=pass_in + "init_tgt1_tgt2.txt",
        return_image=False,
        logging=True,
    )
    ...
    # transform result and save output
    tformed_im = transform_mc_image_sitk(
        target1_fp, tgt1_tgt2_tform, target1_res, override_tform=False
    )

    if check_im_size_fiji(tformed_im) is True:
        sitk.WriteImage(
            tformed_im,
            os.path.join(os.getcwd(), opdir, project_name + "_tgt1_tgt2.mha"),
            True,
        )
    else:
        sitk.WriteImage(
            tformed_im,
            os.path.join(os.getcwd(), opdir, project_name + "_tgt1_tgt2.tif"),
            True,
        )
    ...
    tformed_im = transform_mc_image_sitk(
        tformed_im, tgt1_tgt2_tform, source_res, from_file=False, override_tform=False
    )

    sitk.WriteImage(
        tformed_im,
        os.path.join(os.getcwd(), opdir, project_name + "_src_tgt2.tif"),
        True,
    )
