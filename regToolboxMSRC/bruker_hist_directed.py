...
from regToolboxMSRC.utils.flx_utils import (
    ROIhandler,
    output_flex_rects,
    output_flex_polys,
    split_boxes,
    split_polys,
    mask_contours_to_boxes,
    mask_contours_to_polygons,
)


def bruker_output_xmls(
    source_fp,
    target_fp,
    wd,
    ijroi_fp,
    ...
    if roi_type == "rectangle":
        ...
        rois = transform_mc_image_sitk(
            rois.box_mask, tmap_correction, 1, from_file=False, is_binary_mask=True
        )
    ...
    if roi_type == "polygon":
        ...
        rois.pg_mask = transform_mc_image_sitk(
            rois.pg_mask, tmap_correction, 1, from_file=False, is_binary_mask=True
        )
